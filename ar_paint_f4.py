...
def main():
    ...
    if args['numered_paint'] is False:
        tela = np.ones((size[1], size[0], 3), dtype = np.uint8)*255
    else:
        tela = np.ones((size[1], size[0], 3), dtype = np.uint8)*255

        # Divide a tela em 6 células numeradas e preenche com números aleatórios de 1 a 3
        cell_size = tela.shape[1] // 3  # Dividir horizontalmente em 3 células
        num_cols = 3
        num_rows = 2
        ...
        def draw(event, x, y, flags, param):
            ...
            cv2.circle(tela, (x, y), brush_size, color, -1)
            row = y // cell_size
            col = x // cell_size
            if zones[row, col] == 0:
                zones[row, col] = int(color[0] / 85) + 1  # Mapeia a cor para 1, 2 ou 3
        ...
        def draw_cells(tela):
            for row in range(num_rows):
                for col in range(num_cols):
                    ...
                    center_x = (x1 + x2) // 2
                    center_y = (y1 + y2) // 2
                    # Desenha o número no centro da célula
