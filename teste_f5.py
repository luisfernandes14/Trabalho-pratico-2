...
def evaluate_painting_accuracy(painted_image, zones, expected_colors):
    zone_accuracies = []
    
    for zone_number in range(1, np.max(zones) + 1):
        zone_mask = (zones == zone_number)
        expected_color = expected_colors[zone_number]
        
        # Resize the zone_mask to match the dimensions of the painted_image
        zone_mask_resized = cv2.resize(zone_mask.astype(np.uint8), (painted_image.shape[1], painted_image.shape[0]))

        # Calculate the number of correct pixels
        correct_pixels = np.sum(np.all(painted_image[zone_mask_resized.astype(bool)] == expected_color, axis=-1))
        total_pixels = np.sum(zone_mask_resized)

        accuracy = correct_pixels / total_pixels if total_pixels > 0 else 0
        zone_accuracies.append(accuracy)
    
    return zone_accuracies
...
def main():
    ...
    ## Funcionalidade 5 - Percentagem Pintada por zona
    if args['numered_paint']:
        # Define expected colors for each zone (replace with your actual expected colors)
        expected_colors = {
        1: (255, 0, 0),  # Zone 1 should be red
        2: (0, 255, 0),  # Zone 2 should be green
        3: (0, 0, 255)   # Zone 3 should be blue
        }

    accuracy = evaluate_painting_accuracy(tela, zones, expected_colors)
    print("Painting Accuracy for Each Zone:")
    for zone_number, acc in enumerate(accuracy, start=1):
        print(f"Zone {zone_number}: {acc * 100:.2f}%")    
